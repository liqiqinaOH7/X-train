...
//(x,y):坐标
//r    :半径
void LCD_Draw_ColorCircle(u16 x0,u16 y0,u8 r,u16 color)
{
	int a,b;
	int di;
	a=0;b=r;	  
	...
	while(a<=b)
	{
		int i = a,p = b;
		while(i>0){		
			LCD_DrawColorPoint(x0+b,y0-i,color);
			LCD_DrawColorPoint(x0-i,y0+b,color);
			i--;
	}		
		while(p>0){		
			LCD_DrawColorPoint(x0-a,y0-p,color);
			LCD_DrawColorPoint(x0-p,y0-a,color);
			LCD_DrawColorPoint(x0+a,y0-p,color);
			LCD_DrawColorPoint(x0-p,y0+a,color);
			LCD_DrawColorPoint(x0+a,y0+p,color);
			LCD_DrawColorPoint(x0+p,y0+a,color);
	...
	LCD_DrawColorPoint(x0,y0,color); //圆心坐标
}
//设置指定位置的光标颜色
...
//在指定位置画一个空心半圆
void LCD_Draw_SemiCircle(u16 x0, u16 y0, u8 r)
{
    int a, b;
    int di;
		int i;
    a = 0; b = r;
    di = 3 - (r << 1); // Determine the next point's position flag

    while (a <= b) {
        for (i = a; i >= 0; i--) {
            LCD_DrawPoint(x0 + b, y0 - i); // upper right
            LCD_DrawPoint(x0 - b, y0 - i); // upper left
        }

        for (i = b; i >= 0; i--) {
            LCD_DrawPoint(x0 + a, y0 - i); // right upper
            LCD_DrawPoint(x0 - a, y0 - i); // left upper
        }
...
//在指定位置画一个实心上半圆
void LCD_Draw_FilledUpperSemicircle(u16 x0, u16 y0, u8 r, u16 color)
{
		int a, b;
    int di;
		int i;
    ...
    while (a <= b) {
        for (i = a; i >= 0; i--) {
            LCD_DrawColorPoint(x0 + b, y0 - i, color); // upper right
            LCD_DrawColorPoint(x0 - b, y0 - i, color); // upper left
        }

        for (i = b; i >= 0; i--) {
            LCD_DrawColorPoint(x0 + a, y0 - i, color); // right upper
            LCD_DrawColorPoint(x0 - a, y0 - i, color); // left upper
        }
...
//在指定位置画一个实心下半圆
void LCD_Draw_FilledLowerSemicircle(u16 x0, u16 y0, u8 r, u16 color) 
{
    int a, b;
    int di;
    int i;
    ...
    while (a <= b) {
        for (i = 0; i <= a; i++) {
            LCD_DrawColorPoint(x0 + b, y0 + i, color); // lower right
            LCD_DrawColorPoint(x0 - b, y0 + i, color); // lower left
        }

        for (i = 0; i <= b; i++) {
            LCD_DrawColorPoint(x0 + a, y0 + i, color); // right lower
            LCD_DrawColorPoint(x0 - a, y0 + i, color); // left lower
        }
...
//在指定位置画一个实心半圆
void DrawFilledSemiCircle(int x0, int y0, int radius, u16 color, bool topHalf)
{
    int x = radius;
    int y = 0;
    int err = 0;
    ...
    while (x >= y) {
        for (i = x0 - x; i <= x0 + x; i++) {
            if (topHalf) {
                LCD_DrawColorPoint(i, y0 - y, color); //上半圆
                LCD_DrawColorPoint(i, y0 - x, color);
            } else {
                LCD_DrawColorPoint(i, y0 + y, color); //下半圆
                LCD_DrawColorPoint(i, y0 + x, color);
            }
        }
